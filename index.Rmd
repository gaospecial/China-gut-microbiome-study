...
## 论文年发表量
world_yearly_output <- read.csv("data/Wos-world-yearly-output.csv")
...
plots <- lapply(list(all_article, highlycited), function(d){
  ggplot(d, aes(PY,yearly_output)) +
    scale_fill_gradient2(low = brewer.pal(n=3, name="YlGnBu")[[1]],
                         mid = brewer.pal(n=3, name="YlGnBu")[[2]],
                         high=brewer.pal(n=3, name="YlGnBu")[[3]]) +
    geom_col() +
    theme_light() +
